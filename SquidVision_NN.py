...
def homographical_augment(image_set, truth_set):
    ...
    print(np.shape(augmented_truths))
    print(np.shape(flipped_images))
    print(np.shape(flipped_truths))
...
#    Load Dataset and CSV
    ...
    print("a_t: " + str(np.shape(a_t)))
    print("a_tr: " + str(np.shape(a_tr)))
    ...
    #model = basic_cnn(image_size)
    model = sourced_cnn()
    #model.load_weights('ghetto_net_weights.h5')
    checkpoint = ModelCheckpoint('ghetto_net_weightsv2.h5', monitor='val_accuracy', save_best_only=True, save_weights_only=True, verbose=1)
    model.fit(x_train, y_train, batch_size=16, epochs=100, callbacks=[checkpoint], validation_split=0.14628, verbose=1, shuffle=True)
